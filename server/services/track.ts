...
// ─── Change subscription plan (after provisioning) ───────────────────────────
// After Heimdall creates a workspace (without license details), Heimdall's queue
// creates a customer in tiny-track with the org's default plan. We then change
// the subscription to the correct AppSumo tier plan.
// Retries with backoff because the queue job is async — the customer/subscription
// may not exist yet when we first call.
const CHANGE_PLAN_MAX_RETRIES = 5;
const CHANGE_PLAN_RETRY_DELAY_MS = 3000;

export async function changePlanForWorkspace(
  workspaceId: string,
  planId: string,
  tier: number
): Promise<void> {
  console.log("[Track:changePlan] ═══════════════════════════════════════");
  console.log("[Track:changePlan] Changing subscription plan after provisioning");
  console.log("[Track:changePlan] Workspace (externalId):", workspaceId);
  console.log("[Track:changePlan] Target plan:", planId);
  console.log("[Track:changePlan] Tier:", tier);
  logConfig();

  if (!TRACK_TOKEN) {
    console.error("[Track:changePlan] TRACK_API_TOKEN not configured — skipping");
    throw new TrackError("TRACK_API_TOKEN not configured");
  }

  // Retry loop — the Heimdall queue job may not have created the customer yet
  for (let attempt = 1; attempt <= CHANGE_PLAN_MAX_RETRIES; attempt++) {
    console.log("[Track:changePlan] Attempt", attempt, "of", CHANGE_PLAN_MAX_RETRIES);

    // Step 1: Resolve externalId → customerId
    const customer = await resolveCustomerId(workspaceId);
    if (!customer) {
      if (attempt < CHANGE_PLAN_MAX_RETRIES) {
        console.log("[Track:changePlan] Customer not found yet — waiting", CHANGE_PLAN_RETRY_DELAY_MS, "ms before retry...");
        await new Promise(r => setTimeout(r, CHANGE_PLAN_RETRY_DELAY_MS));
        continue;
      }
      console.error("[Track:changePlan] Customer still not found after", CHANGE_PLAN_MAX_RETRIES, "attempts");
      throw new TrackError(`Customer not found for workspace ${workspaceId} after ${CHANGE_PLAN_MAX_RETRIES} retries`);
    }
    ...
    if (!sub) {
      if (attempt < CHANGE_PLAN_MAX_RETRIES) {
        console.log("[Track:changePlan] Subscription not found yet — waiting", CHANGE_PLAN_RETRY_DELAY_MS, "ms before retry...");
        await new Promise(r => setTimeout(r, CHANGE_PLAN_RETRY_DELAY_MS));
        continue;
      }
      console.error("[Track:changePlan] Subscription still not found after", CHANGE_PLAN_MAX_RETRIES, "attempts");
      throw new TrackError(`Subscription not found for customer ${customer.customerId} after ${CHANGE_PLAN_MAX_RETRIES} retries`);
    }

    // Step 3: If already on the right plan, skip
    if (sub.planId === planId) {
      console.log("[Track:changePlan] Already on correct plan:", planId, "— skipping");
      console.log("[Track:changePlan] ═══════════════════════════════════════");
      return;
    }

    // Step 4: Change plan
    const url = `${TRACK_BASE}/api/subscriptions/${sub.subscriptionId}/change-plan`;
    const headers = authHeaders();
    const requestBody = {
      newPlanId: planId,
      newBillingCycle: "lifetime",
    };

    console.log("[Track:changePlan] Changing plan...");
    console.log("[Track:changePlan] URL:", url);
    console.log("[Track:changePlan] Method: POST");
    logHeaders(headers);
    console.log("[Track:changePlan] Request body:", JSON.stringify(requestBody));
    logCurl("POST", url, headers, JSON.stringify(requestBody));
    ...
    console.log("[Track:changePlan] Response status:", res.status);
    console.log("[Track:changePlan] Response content-type:", res.headers.get("content-type"));

    if (!res.ok) {
      const resBody = await res.text();
      console.error("[Track:changePlan] Response body (error):", resBody.slice(0, 500));
      throw new TrackError(`Failed to change plan: HTTP ${res.status}`);
    }

    const data = (await res.json()) as Record<string, unknown>;
    console.log("[Track:changePlan] Response body:", JSON.stringify(data).slice(0, 800));

    if (data.success === false) {
      throw new TrackError(`Change plan failed: ${(data.message as string) || "unknown error"}`);
    }

    console.log("[Track:changePlan] ═══════════════════════════════════════");
    console.log("[Track:changePlan] SUCCESS — subscription", sub.subscriptionId, "changed to plan", planId, "(tier", tier + ")");
    return;
  }
}
